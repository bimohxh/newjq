...
!function ($) {
	...
	var Datetimepicker = function (element, options) {
		...
		while (template.toString().indexOf('{iconType}') !== -1) {
			template = template.replace('{iconType}', this.icontype);
		}
		while (template.toString().indexOf('{leftArrow}') !== -1) {
			template = template.replace('{leftArrow}', this.icons.leftArrow);
		}
		while (template.toString().indexOf('{rightArrow}') !== -1) {
	...
	Datetimepicker.prototype = {
		...
		fill: function () {
			...
			for (var i = 0; i < 24; i++) {
				if (hoursDisabled.toString().indexOf(i) !== -1) continue;
			...
			for (var i = 0; i < 60; i += this.minuteStep) {
				if (minutesDisabled.toString().indexOf(i) !== -1) continue;
					...
					case 'span':
						if (!target.is('.disabled')) {
							...
							if (target.is('.month')) {
								...
								minutes = parseInt(target.text().substr(target.text().toString().indexOf(':') + 1), 10) || 0;
